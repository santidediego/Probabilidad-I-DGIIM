...
Para la definición anterior, es suficiente con exigir la medibilidad de las imágenes inversas de los elementos de una subclase $\mathcal{a}$ para la cual la $\sigma$-álgebra minimal sobre $\mathcal{a}$ sea $\mathcal{B}$\\
...
Dado un espacio probabilístico $(\Omega, \mathcal{A}, P)$ y una v.a. $\mathbb{X}$, se define la esperanza matemática de $\mathbb{X}$ como:
$$E(\mathbb{X})=\int_{\Omega}\mathbb{X}dP=\int_{\omega\in\Omega}{\mathbb{X}(\omega)dP(\omega)}$$
\\\\
...
$$E(\mathbb{X})=x_1p(\mathbb{X}=x_1)+\ldots + x_n p(\mathbb{X}=x_n)=\sum_{i=1}^{n}{x_i p(x_i)}$$
En el caso de una v.a. continua, sea $f(x)$ la función de densidad. Definimos la esperanza matemática como:
$$E(\mathbb{X})=\int_\infty ^\infty {xf(x)dx}$$
