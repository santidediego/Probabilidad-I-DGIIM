...
\\\\
Una vez visto el centramiento vamos con el \textbf{truncamiento}. Diremos que truncamos $X$ a $c>0$ (finita) cuando reemplazamos $X$ por $X^c=X$ o $0$ dependiendo de si $|X|<c$ o si $|X|\geq c$ ; se dice entonces que $X^c$ es $X$ truncada a $c$.
\\\\
...
\\\\
\textbf{Definición de convergencia equivalente: } Se dice que $X_n$ y $X_n^{\prime}$ son convergentemente equivalentes si solamente convergen en el mismo evento hasta un subconjunto nulo.

\begin{lemma}
Si las series $\sum P[X_n\neq X_n^{\prime}]$ convergen, entonces las sucesiones $X_n$ y $X_n^{\prime}$ son cola-equivalentes, y por tanto, las series $\sum X_n$ y $\sum X_n^{\prime}$ son convergentemente equivalentes y las series $\dfrac{S_n}{b_n}$ y $\dfrac{S_n^{\prime}}{b_n}$ con $b\rightarrow\infty$ convergen al mismo evento y al mismo límite, excluyendo el evento nulo.
\end{lemma}
...
$$\frac{\displaystyle\sum_{k=1}^n \sigma^2 X_k-\epsilon ^2}{sup(S_n-ES_n)^2}\leq P[|S_n-ES_n|\geq \epsilon]\leq \frac{1}{\epsilon^2}\sum_{k=1}^n\sigma^2 X_k$$
Además se tiene el siguiente lema:
\begin{lemma}
Si las series $\sum\sigma^2 X_n$ convergen, entonces las series $\sum (X_n-EX_n)$ convergen en probabilidad. Si $\dfrac{1}{b_n^2}\displaystyle\sum_{k=1}^n \sigma^2 X_k\rightarrow 0$, entonces $\dfrac{S_n - ES_n}{b_n}\stackrel{\mathcal{P}}{\rightarrow}0$
\end{lemma}
Este último lema es debido a Tchebichev (cuando $b_n=n$). En el caso de Bernouilli, donde $b_n=n$, $EX_n=p$ , $\sigma^2 X_n=pq$ se reduce a la ley de Bernouilli de los grandes números.
\end{document}
