...
\author{Santiago de Diego\\Braulio Valdivielso\\Francisco Luque\\Pedro Bonilla}
\title{Probabilidad I}
\date{}
...
donde $X^{+} = XI_{[X\geq0]}$ y $X^{-} = -XI_{[X<0]}$. Si $\displaystyle \int_{\Omega}Xd\mathcal{P}$ es finita, es decir, si los dos términos de la diferencia anterior son finitos, entonces se dice que $X$ es integrable en $\Omega$. Ahora, una vez definida la integral, vamos a ver algunas de sus propiedades. Tenemos primero una serie de propiedades relacionadas con la aditividad de la integral. \\


\begin{theorem} Sean $X,Y$ dos funciones medibles, entonces :
...
\textbf{Corolario 1: } $\displaystyle \int_{A+B} X d\mathcal{P} = \int_A X d\mathcal{P} + \int_B X d \mathcal{P}$\\


\textbf{Corolario 2: } $\displaystyle \int cXd\mathcal{P} = c\int Xd\mathcal{P}$\\

Por último veamos ahora algunas propiedades relacionadas con el orden:\\
\begin{itemize}
	
\item $X \geq 0 \Rightarrow \displaystyle \int X d\mathcal{P} \geq \int 0 = 0$\\

\item $X \geq Y \Rightarrow \displaystyle \int X d\mathcal{P} \geq \int Y d\mathcal{P}$\\

\item $\displaystyle X \stackrel{c.s}{=} Y \Rightarrow \int X d\mathcal{P} = \int Y d\mathcal{P}$\\
