...
$$ \Rightarrow \mathcal{P}[\mid X_{n+u} - X_n \mid \varepsilon] \leq \mathcal{P}\Big[\mid X_{n+u} - X \mid \frac{\varepsilon}{2}\Big] + \mathcal{P}\Big[\mid X_{n} - X \mid \frac{\varepsilon}{2}\Big] \to 0$$
\end{proof}
...
\begin{lemma}
Sea $Y\leq \mathbb {X}_n\leq Z$ con $Y,\, Z$ integrables y tal que $X_n\overbrace{\longrightarrow}^{c.s.}X$. Entonces se tiene que $\displaystyle \int X_n \rightarrow \displaystyle \int X$
\end{lemma}
...
\textbf{Definición de convergencia casi segura:} Una sucesión de variables aleatorias, ${ X_n }$ , converge con probabilidad 1, o de forma casi segura, a una variable aleatoria $X$ ( que puede degenerar en una constante K) cuando se cumple que:
$$P(\lim_{n\rightarrow\infty}X_n=X)=1$$
De esta forma interpretamos que $X_n\overbrace{\longrightarrow}^{c.s.}X$ cuando la probabilidad de que en el límite la sucesión de variables  aleatorias y aquella a la que converge sean iguales es uno

\begin{theorem}
$$X_n\overbrace{\longrightarrow}^{c.s.}X\Longrightarrow X_n\stackrel{\mathcal{P}}{\longrightarrow}X$$
$$X_n\stackrel{\mathcal{P}}{\longrightarrow}X\Longrightarrow \exists X_{nk}\,:\, X_{n_k}\overbrace{\longrightarrow}^{c.s.}_{k\rightarrow\infty}X$$
\end{theorem}
\begin{proof}
