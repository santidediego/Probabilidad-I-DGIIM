...
\\\\
\begin{itemizw}
\item Sea $(\Omega,a,P) \, , \, A_n \in a$. Entonces, si $\displaystyle\sum P \, A_n < \infty \Longrightarrow P(\displaystyle \lim_{sup}A_n)=0$
\item ${A_n}$ independientes $\Longrightarrow P(\displaystyle\lim_{sup}(A_n))=0$ si $\sum P(A_k)<\infty$ o 1 si $\sum P(A_k)=\infty$
\end{itemize}
\end{lemma}
\begin{proof}
...
\end{proof}
% Recordatorio de función caracteristica (meter arriba)
...
\end{proof}
% Introducir corolarios de la ley 0-1 de Borel
...
\begin{proof}
% HACER DEMOSTRACIÓN, IMPORTANTE (MOJO)
\end{proof}
% Añadir corolario del la ley 0-1 de Kolmogorov
...
\begin{enumerate}
% AÑADIR LEYES DE MARIADEL (PAGINA 35)
