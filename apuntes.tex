...
De hecho, el concepto de independencia es relativo a la familia de clases. Una clase de eventos se dice independiente si sus eventos son independientes. Además se tiene que las subclases de una clase independiente son independientes.
...
\begin{theorem}
Los $\sigma$ campos independientes sobre clases independientes cerradas para la intersección finita son independientes
\end{theorem}
...
$P\bigcup_{k=1}^{n}[X_{t_k}\in S{t_k}]=\displaystyle\prod_{k=1}^n P[X_{t_k}\in S_{t_k}]$
...
\begin{theorem}
Las tres definiciones siguientes de independencia de variables aleatorias $X_t$ son equivalentes:
\begin{itemize}
\item $P\bigcup_{k=1}^n [X_{tk}\in S_{tk}]=\displaystyle\prod_{k=1}^n P[X_{tk}\in S_{tk}]$
...
\end{proof}
\subsubsection{Criterio no-cero de Borel}
Si los eventos $A_n$ son independientes, entonces $P(\displaystyle\lim_{sup}A_n)=0$ o 1 dependiendo de si $\sum P A_n<\infty$ o $\sum P A_n=\infty$ 
\begin{proof}
...
Esta sección está dedicada a la investigación de las sumas de variables aleatorias independientes y especialmente, a sus propiedades en el límite.
\\\\
...
\begin{lemma}
Si las series $\sum P[X_n\neq X_n^{\prime}]$ convergen, entonces las sucesiones $X_n$ y $X_n^{\prime}$ son cola-equivalentes, y por tanto, las series $\sum X_n$ y $\sum X_n^{\prime}$ son equivalentes en la convergencia y las series $\dfrac{S_n}{b_n}$ y $\dfrac{S_n^{\prime}}{b_n}$ con $b\rightarrow\infty$ convergen al mismo evento y al mismo límite, excluyendo el evento nulo.
\end{lemma}
...
Con la desigualdad anterior obtenemos la siguiente:
...
Además se tiene el siguiente corolario:
\\\\
\textbf{Corolario: }Si las series $\sum\sigma^2 X_n$ convergen, entonces las series $\sum (X_n-EX_n)$ convergen en probabilidad. Si $\dfrac{1}{b_n^2}\displaystyle\sum_{k=1}^n \sigma^2 X_k\rightarrow 0$, entonces $\dfrac{S_n - ES_n}{b_n}\stackrel{\mathcal{P}}{\rightarrow}0$
\\\\
\begin{proof}
...
\chapter{El problema del límite central}
...
\\\\
ATENCION: ESTO HAY QUE PONERLO BIEN, ESPERAR A CLASE!!!
\begin{enumerate}
\item La ley de degeneración $\delta(0)$ de una variable aleatoria degenera a 0 teniendo un punto de incremento solo en $x=0$
...
\section{Primer teorema del límite central}
En el caso de Bernouilli:
$$\delta\left( \frac{S_n-ES_n}{n}  \right)\rightarrow \delta(0) \,\, y \,\, \delta\left( \frac{S_n-ES_n}{\sigma S_n}  \right)\rightarrow N(0,1)$$
y en el caso de Poisson:
$$\delta(S_{nn})\rightarrow P(\lambda)$$
\begin{proof}
...
\begin{enumerate}
\item Si $\dfrac{1}{n^2}\displaystyle\sum_{k=1}^n E|X_{nk}|^2\rightarrow 0$ entonces $\delta \left( \dfrac{S_{nn}}{n} \right)\rightarrow \delta(0)$
\item Si $\dfrac{1}{s_{nn}^3}\displaystyle\sum_{k=1}^n E|X_{nk}|^3\rightarrow 0$ entonces $\delta \left( \dfrac{S_{nn}}{s_nn} \right)\rightarrow N(0,1)$
\end{enumerate}
\end{lemma}
\subsubsection{Lema de equivalencia de leyes}
\begin{lemma}
Si $X_n - X\stackrel{\mathbb{P}}{\longrightarrow} 0$ o $P[X_n \neq X_n^{\prime}]\longrightarrow 0$ entonces las sucesiones $\delta(X_n)$ y $delta(N_n^{prime})$ son equivalentes.
\end{lemma}
...
Se tiene que $\delta \left( \dfrac{S_n}{n} \right)\rightarrow \delta(0)$ si y solamente si:
\begin{enumerate}
...
Se tiene $\delta \left( \dfrac{S_n}{s_n} \right)\rightarrow N(0,1)$ y $max_{k\leq n}\dfrac{\sigma_k}{s_n}\rightarrow 0$ si y solamente si, $\forall \epsilon >0$:
$$g_n(\epsilon)=\dfrac{1}{s_n^2}\sum\int_{|x|\geq \epsilon s_n}x^2\, dF_k\longrightarrow 0$$
\begin{proof}

\end{proof}
\section{Aproximación normal}
En sus investigaciones sobre la convergencia normal, Liapunov no solo examinó las condiciones para la velocidad de la convergencia. Sus resultados fueron mejorados por Berry y nos dicen lo básico sobre como debemos proceder por pasos.
\\\\
Sean $F$ y $G$ dos funciones de densidad de variables aleatorias con sus correspondientes funciones características $f$ y $g$ y sea $H=F-G$ , $h=f-g$. Excluimos el caso trivial en el que $\alpha=sup|h|$ , esto es, $H=h=0$. Tenemos entonces dos resultados
\begin{enumerate}
\item Si $G$ es continua en $R$, entonces existe un número finito $s$ tal que o bien $H(s)=\pm \alpha$ o $H(s+0)=\alpha$
\item Si $G$ tiene una derivada $G^{\prime}$ en $R$, entonces existe un número finito $a$ tal que:
$$\left| \int H(x+a)p(x)dx \right| \geq \frac{\alpha}{2}\left( 1-6\int_{\frac{\alpha}{2\beta}}^{\infty}p(x)dx \right)\,\,\, , \,\,\, \beta=sup|G^{\prime}|$$
\item Para todo $a\in R$:
$$\frac{1}{2\pi}\int \left| \frac{h(u)\omega(u)}{u}du \right|\geq \left| \int H(x+a)p(x)dx  \right|$$
\end{enumerate}
\subsubsection{Desigualdad básica}
Si $G$ tiene derivada $G^{prime}$ en $R$, entonces $\forall U>0$,
$$sup |H|\leq\dfrac{2}{\pi}\int_{0}^{u}\left| \frac{h(u)}{u} \right| du + \frac{24}{\pi U}sup |G^{\prime}|$$
\begin{proof}

\end{proof}
\subsubsection{Teorema de aproximación normal}
\end{document}
